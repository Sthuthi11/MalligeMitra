...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    loginForm?.addEventListener('submit', (event: SubmitEvent) => {
        ...
        const messageBox = document.createElement('div');
        messageBox.className = 'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50';
        messageBox.innerHTML = `<div class="bg-white p-6 rounded-lg shadow-xl text-center max-w-sm mx-auto"><p class="text-lg font-semibold text-gray-800 mb-4">Login attempt received!</p><p class="text-gray-600 mb-2">Username: ${username}</p><p class="text-gray-600 mb-4">Password: ${'*'.repeat(password.length)}</p><p class="text-xs text-gray-500 mb-4">(In a real app, this would be authenticated)</p><button id="close-message-login" class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-md shadow-md">Close</button></div>`;
        document.body.appendChild(messageBox);
        document.getElementById('close-message-login')?.addEventListener('click', () => messageBox.remove());
    });
    ...
    <>
      ...
      <header className="app-header shadow-sm py-4 px-6 md:px-10 flex justify-between items-center">
        ...
        <nav className="flex items-center space-x-4">
          <a href="#" id="nav-home" className="text-white hover:text-green-200 px-3 py-2 rounded-md transition-colors duration-200">Home</a>
      ...
      <main id="dynamic-content-area" className="flex-grow container mx-auto p-4 md:p-8 flex flex-col items-center justify-start relative z-10">
        ...
        <section id="login-page-section" className="page-section w-full max-w-md hidden">
             <div className="bg-white rounded-xl shadow-lg p-6 md:p-10 w-full">
                ...
                <form id="login-form" className="space-y-6">
                    ...
                    <p className="text-center text-sm text-gray-600 mt-4">
                        Don&apos;t have an account? <a href="#" id="login-to-register" className="text-green-600 hover:underline">Register here</a>
                    </p>
                </form>
            </div>
        </section>
